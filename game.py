...
START_CASH = 10000.0; START_ASSETS = 25000.0; CAP_NORM = 100.0; DEPRECIATION = 0.017
STORAGE_COST = 1.5; MATERIAL_UNIT = 6.0; FIXED_COST = 1000.0
DEPOSIT_RATE = 0.008; LOAN_RATE = 0.015; LOAN_LIMIT = 50000.0; START_PRICE = 30.0
QUALITY_DECAY = 3.0; RND_GAIN = 0.001
...
NOVICE_WAGE = 60; TRAIN_MONTHS = 3; TRAIN_COST = 1200; MENTOR_BONUS = 0.5; MANAGER_SPAN = 8
...
EVENTS = [
    ...
    (0.03,"Скачок цен на сырьё: затраты +20%.",1.0,1.2,0,None,None),
    (0.03,"Сырьё подешевело: затраты −10%.",1.0,0.9,0,None,None),
    (0.04,"Ставка ЦБ выросла: кредиты дороже, депозиты выгоднее.",1.0,1.0,1,"negative",None),
    (0.04,"Ставка ЦБ снизилась: кредиты дешевле.",1.0,1.0,-1,"positive",None),
    (0.03,"Пожар на заводе Hengda Paint (Китай): покрытия +15%.",1.0,1.15,0,None,None),
    (0.02,"Обязательная маркировка игрушек: затраты +3%.",1.0,1.03,0,None,None),
    (0.03,"Пожары в Сибири: дефицит липы и берёзы, сырьё +8%.",1.0,1.08,0,None,None),
    (0.03,"Запрет пластиковых игрушек: покупатели переходят на деревянные!",1.35,1.0,0,"positive",None),
    (0.03,"Рождаемость +8%: спрос на деревянные игрушки растёт.",1.25,1.0,0,"positive",None),
    (0.03,"Фестиваль «Матрёшка-фест»: всплеск интереса к сувенирам!",1.2,1.0,0,"positive",None),
    (0.02,"Контрафакт наводнил маркетплейсы подделками.",0.9,1.0,0,None,None),
    (0.03,"Экспортный контракт с Китаем: спрос ×1.4.",1.4,1.0,0,"positive",None),
    (0.02,"Забастовка перевозчиков: логистика +10%.",1.0,1.10,0,None,None),
    ...
    (0.02,"Новый ГОСТ на деревянные игрушки: затраты +5%.",1.0,1.05,0,None,"gost"),
    (0.02,"Сезонный грипп: персонал на больничных, производительность вниз.",1.0,1.0,0,None,"sick"),
    (0.03,"Импортные лаки и краски дорожают: покрытия +8%.",1.0,1.08,0,None,None),
    (0.03,"Лесная биржа: штабели липы подешевели, сырьё −10%.",1.0,0.90,0,None,None),
    (0.03,"Запуск маркетплейса «Деревянные игрушки России»: онлайн-канал на 2 месяца, качество ≥100 получает +10% спроса.",1.0,1.0,0,"positive","marketplace"),
    (0.03,"Десант блогеров в округ: медиаволна, маркетинг всех компаний эффективнее на 20%.",1.0,1.0,0,"positive","bloggers"),
    (0.03,"Засуха: усыхание липы, сырьё +12% на месяц.",1.0,1.12,0,None,None),
    (0.03,"Экспортное окно: рубль ослаб, спрос на экспортные партии ×1.3.",1.0,1.0,0,"positive","exportwin"),
    (0.03,"Форум «Промысел и бизнес»: можно оплатить участие (−касса, +1 репутация).",1.0,1.0,0,None,"forum"),
    (0.02,"Объявлен Год народной культуры: полгода спрос на сувениры +15%.",1.0,1.0,0,"positive","folkyr"),
    (0.02,"Скандал: травма от некачественной игрушки — спрос −10%, но качественным (≥110) доверяют больше.",0.9,1.0,0,None,"scandal"),
    (0.03,"Карантин по короеду: сырьё +10% (компании с фьючерсом не страдают).",1.0,1.10,0,None,"quarantine"),
    (0.02,"Перепись покупателей сувениров: следующие 3 месяца прогноз редакции точен на 100%.",1.0,1.0,0,None,"census"),
    (0.03,"Дорожает энергия: затраты +6% на месяц.",1.0,1.06,0,None,None),
    (0.03,"Уроки промысла в школах: спрос на дешёвый сегмент вырос.",1.0,1.0,0,None,"lessons"),
]
PERSONAL_EVENTS = ["recall","viral","master_leave","corp_order","warehouse_accident","roscachestvo",
    "breakdown","contest","defect","review","rational","museum","delay","taxerr",
    "gift_region","fireinspect","wedding","union_premium"]
...
BOT_STRATS = ["conservative","aggressive","balanced","dumper","niche","financier","adaptive","expansionist","follower"]
ACH_DEFS = [("first_profit","Первая прибыль"),("first_patent","Первый патент"),("first_tender","Первый госконтракт"),
    ("survived_strike","Пережил забастовку"),("mecenate","Меценат"),("brand_holder","Носитель бренда"),
    ("record_setter","Рекордсмен отрасли"),("big_employer","Крупный работодатель"),("master_quality","Мастер качества (≥120)"),
    ("millionaire","Миллионер"),("clean_quarter","Чистый квартал"),("half_market","Половина рынка"),("sold_out","Всё продано"),
    ("exporter","Экспортёр"),("reliable_supplier","Надёжный поставщик"),("full_staff","Полный штат"),
    ("calm_collective","Спокойный коллектив"),("peoples_love","Народная любовь"),("quality_150","Качество 150"),
...
const CARDS = {'card-finance':['#1f6fb2','#17568c'],'card-price':['#1a7a4a','#135c38'],'card-charity':['#e8b30a','#b0880a'],'card-growth':['#e07b1f','#b05e14'],'card-subsidy':['#462446','#331a33'],'card-hr':['#0e7c86','#0a5a62'],'card-tender':['#7a1f3d','#5a152c'],'card-risk':['#5a4632','#3d2f21']};
const PANELS = {'panel-finance':['#dbe9f6','#17568c'],'panel-price':['#d9ead9','#135c38'],'panel-charity':['#fdf3d0','#8a6d05'],'panel-growth':['#fbe3c8','#b05e14'],'panel-subsidy':['#e6dcec','#462446'],'panel-hr':['#d7ecee','#0a5a62'],'panel-tender':['#f3dce4','#5a152c'],'panel-risk':['#e8e0d6','#3d2f21']};
function paint(){
...
def photo_b64(base, _depth=0):
    ...
    val = None; fn = _find_file(base)
    if fn:
        path = os.path.join(IMG_DIR, fn); ext = "."+fn.lower().rsplit(".",1)[1]
...
def photo_for(n, front=False):
    ...
    if any(k in t for k in ["отзыв парти","брак у поставщика"]): return "photo_recall"
    if any(k in t for k in ["вирусн","ролик","обзор блогера"]): return "photo_viral"
...
def calendar_html(turn):
    year = (turn-1)//12+1; cur_m = ((turn-1)%12)+1; cells = ""
    for m in range(1,13):
        t_of_m = (year-1)*12+m
...
def clip_html(n, month_label, front=False):
    ...
    base = photo_for(n, front); b64 = photo_b64(base)
    if b64:
        h_px = 190 if front else 140; ratio = photo_ratio(base)
...
def rank_html(game, viewer_name):
    rows = sorted([c for c in game["companies"] if c.history and not getattr(c,"absorbed",False)], key=lambda c: c.mpi_total, reverse=True)
...
def save_game(game, slot):
    ...
    with open(os.path.join(SAVE_DIR, f"{slot}_{st.session_state.token}.json"), "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=1)
def load_game(slot):
    with open(os.path.join(SAVE_DIR, f"{slot}_{st.session_state.token}.json"), encoding="utf-8") as f:
        data = json.load(f)
    data["companies"] = [Company.from_dict(c) for c in data.pop("companies")]
    data.pop("saved_at", None)
    for k, v in {"news_log":[],"next_event":None,"forecast_dir":None,"cost_factor":1.0,"loan_rate":LOAN_RATE,
                 "deposit_rate":DEPOSIT_RATE,"record_sales":0,"record_company":"","record_period":"","patents_log":[],
    ...
    return data
def list_saves():
    suf = f"_{st.session_state.token}.json"
    return sorted(f[:-len(suf)] for f in os.listdir(SAVE_DIR) if f.endswith(suf))
def hof_load():
...
class Company:
    ...
    def unit_cost(self, volume):
        ...
        return uc
    def to_dict(self):
        return dict(name=self.name,is_bot=self.is_bot,strategy=self.strategy,tax=self.tax,cash=self.cash,
            assets=self.assets,inventory=self.inventory,quality=self.quality,patent=self.patent,cum_rnd=self.cum_rnd,
            loans=self.loans,deposits=self.deposits,neg_turns=self.neg_turns,bankrupt=self.bankrupt,
            reputation=self.reputation,mpi_total=self.mpi_total,history=self.history,rescue_taken=self.rescue_taken,
            sub_won=self.sub_won,sub_type=self.sub_type,sub_msg=self.sub_msg,brand=self.brand,pr_next=self.pr_next,
            staff=self.staff,skill=self.skill,novices=self.novices,training=self.training,premium=self.premium,
            loyalty=self.loyalty,workers=self.workers,vacancies=self.vacancies,vac_pct=self.vac_pct,
...
def corp_points(pay): return 50 + min(50, pay/100)
...
def bot_decision(c, avg_price, season, month, difficulty, last_factor, sc, open_types, pool, game):
    ...
    poor = c.cash < sc*0.5
    base_prod = int(cap*0.85*season)
    if c.history:
        base_prod = int(min(cap, c.history[-1]["orders"]*season*1.05))
    price, prod = avg_price, base_prod
    if s == "conservative": price, prod = avg_price*0.95, int(base_prod*0.85)
    elif s == "aggressive": price, prod = avg_price*1.10, min(cap, int(base_prod*1.1))
    elif s == "dumper": price, prod = avg_price*0.85, min(cap, int(base_prod*1.15))
    elif s == "niche": price, prod = avg_price*1.35, int(base_prod*0.6)
    elif s == "financier": price, prod = avg_price*1.05, int(base_prod*0.6)
    elif s == "expansionist": price, prod = avg_price*0.95, min(cap, int(base_prod*1.1))
    elif s == "follower": prod = int(base_prod*0.85)
    est_labor = c.labor_cost()
    reserve = 2*(FIXED_COST+est_labor)
    free = c.cash - reserve
    last_profit = c.history[-1]["profit"] if c.history else 0.0
    envelope = max(0.0, min(free*0.35, 4*max(0.0,last_profit)+800))
    ...
    prio = {"conservative":[("charity",0.20),("rnd",0.30),("marketing",0.30),("invest",0.20)],
            ...
            "follower":[("charity",0.10),("marketing",0.30),("rnd",0.20),("invest",0.10)]}.get(s,[("marketing",0.4),("rnd",0.3),("charity",0.1),("invest",0.2)])
    spend = {"marketing":0.0,"rnd":0.0,"invest":0.0,"charity":0.0}
    for kind, share in prio:
        amt = envelope*share
        if kind == "invest" and not gate_invest: continue
        if kind == "marketing": amt = min(amt, 0.20*est_rev)
        if kind == "rnd": amt = min(amt, 0.06*est_rev)
        spend[kind] += amt
    ...
    if difficulty == "hard" and free > 0:
        if c.cum_rnd < PATENT_THRESHOLD and c.cash > sc*0.8:
            d["rnd"] = max(d["rnd"], 1500.0)
        if not c.brand and c.cash > sc*0.9 and last_profit > 0:
            d["rnd"] = max(d["rnd"], 2500.0)
            d["marketing"] = max(d["marketing"], 1500)
            d["charity"] = max(d["charity"], min(800.0, last_profit*0.3))
        if c.quality < 110 and last_profit > 1500:
            d["rnd"] += 1000
            d["premium"] = 15
    ...
    d["futures"] = (c.futures == 0 and c.cash > sc*0.8 and random.random() < 0.15)
    d["technicum"] = (c.cash > sc*0.7 and random.random() < 0.2)
    d["forum_pay"] = bool(game.get("forum_open")) and c.cash > sc*0.6 and random.random() < 0.5
    if c.history and len(c.history) >= 2:
        prev_sh = c.history[-2]["market_share"]; cur_sh = c.history[-1]["market_share"]
        if cur_sh < prev_sh*0.9:
            d["price"] *= 0.96; d["marketing"] *= 1.25
    d["price"] = min(d["price"], game["avg_price"]+15)
    return d
def run_turn(game, decisions):
    ...
    game["last_factor"] = factor; game["cost_factor"] = cost_factor
    if rate_delta:
    ...
    for _k in ("marketplace","bloggers","exportwin","scandal","lessons","census"):
        if game.get(_k): game[_k] = max(0, game[_k]-1)
    ...
    for c in game["companies"]:
        ...
        if c.demand_mod_months > 0:
            c.demand_mod_months -= 1
    ...
    elif special == "folkyr": game["folk_year"] = 6
    elif special == "scandal": game["scandal"] = 1
    elif special == "census": game["census"] = 3
    elif special == "lessons": game["lessons"] = 1
    game["forum_open"] = (special == "forum")
    mkt_boost = 1.2 if game.get("bloggers") else 1.0
    patent_n = sum(1 for c in alive if c.patent)
    if patent_n >= 2 and not game.get("crosslicense"):
    ...
    if game["turn"] >= 3 and not game.get("tender") and next_month in TENDER_MONTHS:
        ...
        game["tender"] = dict(customer=cname, monthly_vol=vol, price_cap=round(game["avg_price"]*pcap,1), quality_min=qmin, open=True)
        add("🏛", f"Анонс: в следующем месяце откроется тендер госзакупок {cname}. Объём {vol} ед./мес на 3 мес, потолок {game['tender']['price_cap']:.1f} ₽, качество ≥{qmin}.", "gold", "Госзакупки")
    if month in SUBSIDY_MONTHS:
        add("🏛", f"Открыто окно заявок на субсидии Минпромторга: {SUB_TYPE_NAMES[open_types[0]]} и {SUB_TYPE_NAMES[open_types[1]]}.", "gold", "Минпромторг")
    ...
    if month in SUBSIDY_MONTHS:
        ...
        n = len(apps)
        if n == 0: add("🏛", "Конкурс Минпромторга завершился: заявок не подано.", "warn", "Минпромторг")
        elif n == 1:
    ...
    if (game["turn"] >= 5 and not game.get("merger_done") and len(bots_alive) >= 5 and random.random() < 0.12):
        ...
        if absorber is not target:
            ...
            absorber.cash += target.cash; absorber.assets += target.assets; absorber.inventory += target.inventory
            for p in PROFS: absorber.staff[p] += target.staff[p]
            absorber.novices += target.novices; absorber.training += target.training; absorber.contracts += target.contracts
            ...
            integ = absorber.cash*0.05; absorber.cash -= integ
    ...
    if alive and random.random() < 0.35:
        ...
        if pe == "recall":
            ...
            if pc.staff["painters"] > 0 and pc.skill["painters"] >= 3:
                ...
                if rivals: random.choice(rivals).skill["painters"] = min(5.0, random.choice(rivals).skill["painters"]+0.5)
                add("⚠️", f"Уход ключевого мастера из «{pc.name}»: навык росписи −0.5, конкурент переманил его.", "warn", "Уход ключевого мастера")
        elif pe == "corp_order":
            qty = int(pc.capacity()*0.3); rev = qty*(pc.history[-1]["price"] if pc.history else 30)
            ...
            loss = int(pc.inventory*random.uniform(0.2,0.3)); pc.inventory = max(0, pc.inventory-loss)
            add("⚠️", f"Складская авария у «{pc.name}»: потеряно {loss} ед. запасов.", "warn", "Складская авария")
        elif pe == "roscachestvo":
            if pc.quality >= 110:
                pc.reputation += 1; add("✅", f"Проверка Роскачества у «{pc.name}»: пройдено, +1 репутация.", "ok", "Проверка Роскачества")
            else:
                pc.cash -= 1500; add("⚠️", f"Проверка Роскачества у «{pc.name}»: не пройдено, штраф 1 500 ₽.", "warn", "Проверка Роскачества")
        elif pe == "breakdown":
            if pc.cash > 8000:
                pc.cash -= 1500; add("⚙️", f"Поломка оборудования у «{pc.name}»: срочный ремонт за 1 500 ₽, простоя нет.", "warn", "Поломка оборудования")
            else:
                pc.cap_mod = 0.8; pc.cap_mod_months = 1
                add("⚙️", f"Поломка оборудования у «{pc.name}»: плановый ремонт, мощность −20% на месяц.", "warn", "Поломка оборудования")
        elif pe == "contest":
            pc.quality = min(150, pc.quality+3); pc.reputation += 1
            add("🏅", f"Мастер «{pc.name}» взял приз на конкурсе: +3 качества, +1 репутация.", "ok", "Приз на конкурсе")
        elif pe == "defect":
            pc.cash -= (pc.history[-1]["prod_cost"]*0.08 if pc.history else 300)
            add("⚠️", f"Брак у поставщика сырья у «{pc.name}»: затраты на сырьё +8% в этом месяце.", "warn", "Брак у поставщика")
        elif pe == "review":
            if pc.quality >= 110:
                pc.demand_mod = 1.15; pc.demand_mod_months = 2
                add("📱", f"Восторженный обзор блогера на «{pc.name}»: +15% спроса на 2 мес.", "ok", "Обзор блогера")
            else:
                pc.demand_mod = 0.90; pc.demand_mod_months = 1
                add("📱", f"Разгромный обзор блогера на «{pc.name}»: −10% спроса на месяц.", "warn", "Обзор блогера")
        elif pe == "rational":
            if pc.cash > 10000:
                pc.cash -= 1000; pc.rational = True
                add("💡", f"Рацпредложение рабочего на «{pc.name}»: премия 1 000 ₽, себестоимость −5% навсегда.", "ok", "Рацпредложение")
            else:
                pc.tension = min(100, pc.tension+8); add("💡", f"На «{pc.name}» проигнорировали рацпредложение: напряжённость +8.", "warn", "Рацпредложение")
        elif pe == "museum":
            if pc.quality >= 115:
                qty = int(pc.capacity()*0.2); rev = qty*((pc.history[-1]["price"]*1.4) if pc.history else 45)
                pc.cash += rev; pc.inventory = max(0, pc.inventory-qty)
                add("🏛", f"Предзаказ музея для «{pc.name}»: премиальная партия {qty} ед. на {rev:,.0f} ₽.", "ok", "Предзаказ музея")
        elif pe == "delay":
            loss = int((pc.history[-1]["sales"] if pc.history else 0)*0.15); pc.inventory += loss
            add("🚚", f"Срыв перевозки у «{pc.name}»: {loss} ед. задержались и ушли на склад.", "warn", "Срыв перевозки")
        elif pe == "taxerr":
            fine = 0 if pc.reputation >= 5 else 1200; pc.cash -= fine
            add("🧾", f"Налоговая ошибка у «{pc.name}»: "+("репутация спасла от штрафа." if fine == 0 else f"штраф {fine:,.0f} ₽."), "warn", "Налоговая ошибка")
        elif pe == "gift_region":
            pc.reputation += 1; pc.demand_mod = 1.15; pc.demand_mod_months = 2
            add("🎁", f"Матрёшку «{pc.name}» выбрали подарком области: +1 репутация, +15% спроса на 2 мес.", "ok", "Подарок области")
        elif pe == "fireinspect":
            if pc.cash > 6000:
                pc.cash -= 1200; add("🚒", f"Пожарный инспектор у «{pc.name}»: нарушения устранены за 1 200 ₽.", "warn", "Пожарная проверка")
            else:
                pc.cash -= 3000; add("🚒", f"Пожарный инспектор у «{pc.name}»: штраф 3 000 ₽.", "warn", "Пожарная проверка")
        elif pe == "wedding":
            if pc.cash > 5000:
                pc.cash -= 800; pc.tension = max(0, pc.tension-6)
                add("💐", f"«{pc.name}» поздравила ключевого сотрудника: −6 напряжённости.", "ok", "Кадровый праздник")
            else:
                pc.tension = min(100, pc.tension+6); add("💐", f"На «{pc.name}» проигнорировали юбилей сотрудника: +6 напряжённости.", "warn", "Кадровый праздник")
        elif pe == "union_premium":
            if pc.tension > 60 and pc.cash > 5000:
                pc.cash -= 1500; pc.tension = max(0, pc.tension-12)
                add("🧧", f"Профсоюзная ячейка «{pc.name}»: праздничная премия 1 500 ₽, −12 напряжённости.", "ok", "Профсоюзная премия")
            else:
                pc.tension = min(100, pc.tension+10); add("🧧", f"Профсоюзная ячейка «{pc.name}» не получила премию: +10 напряжённости.", "warn", "Профсоюзная премия")
    attr = []
    for c in alive:
        dd = decisions[c.name]
        rep_mult = min(1.0+REP_MKT_CAP, 1.0+REP_MKT*c.reputation)
        ...
        if brand_now and not c.brand: add("®", f"«{c.name}» получила право знака «Вознесенская матрёшка».", "ok", "Региональный бренд")
        if not brand_now and c.brand: add("®", f"«{c.name}» приостановила знак «Вознесенская матрёшка».", "warn", "Региональный бренд")
        c.brand = brand_now
        attr.append(max(0.1, a))
    total_attr = sum(attr)
    total_charity = 0.0; charity_lines = []
    ...
    for i, c in enumerate(alive):
        ...
        if c.strike > 0:
            ...
            if c.strike == 0:
                ...
                c.quality = max(20, c.quality-3); c.tension = max(0, c.tension-30)
        ...
        for ct in list(c.contracts):
            ...
            if deliver < need_v:
                ...
                c.reliability += 1
            if ct["months"] <= 0:
                c.contracts.remove(ct); add("✅", f"«{c.name}» исполнила контракт {ct['customer']}: +1 репутация.", "ok", "Госзакупки")
        sales = min(int(orders), avail)
        lost = max(0.0, orders-sales); c.inventory = avail-sales
        ...
        if charity > 0:
            ...
            if charity >= 1000 and random.random() < 0.4:
                c.pr_next = True; add("♥", f"«{c.name}» получила тёплую заметку за щедрость: +2% спроса в след. месяце.", "ok", "Благотворительная ведомость")
        ...
        bankrupt_now = False
        if c.neg_turns == 2: add("⚠️", f"«{c.name}»: деньги в минусе 2-й месяц — ещё один и банкротство!", "warn")
        if c.neg_turns >= 3 and not (game.get("sandbox") and game["turn"] <= 6):
            if not c.restructured and (c.loans or c.cash < 0):
                c.restructured = True
                c.loans = [[l[0]*0.5, l[1], l[2], l[3]] for l in c.loans]
                c.cash = max(c.cash, 2000); c.neg_turns = 0
                c.reputation = max(0, c.reputation-2); c.reliability = max(0, c.reliability-2)
                add("🛠", f"«{c.name}» прошла реструктуризацию: списано 50% долгов, −2 репутация, −2 надёжность. Второй шанс.", "warn", "Реструктуризация")
            else:
    ...
    if tnd and tnd.get("open") and month in TENDER_MONTHS:
        ...
        if bids:
            bids.sort(key=lambda x: -x[0]); sc_, wc, wprice = bids[0]
            total = wprice*tnd["monthly_vol"]*3; advance = total*TENDER_ADVANCE
            wc.cash += advance
            wc.contracts.append(dict(months=3, monthly_vol=tnd["monthly_vol"], price=wprice, customer=tnd["customer"], total=total))
    ...
    pool["assemblers"] = pool.get("assemblers",0)+2; pool["turners"] = pool.get("turners",0)+1
    ...
    if top_sales.history[-1]["sales"] > 0: add("🏆", f"«{top_sales.name}» — лучшие продажи месяца: {int(top_sales.history[-1]['sales'])} ед.", "gold")
    top_rev = max(hl, key=lambda c: c.history[-1]["revenue"])
    if top_rev.history[-1]["revenue"] > 0: add("💰", f"«{top_rev.name}» — рекордная выручка месяца: {top_rev.history[-1]['revenue']:,.0f} ₽.", "gold")
    worst = min(hl, key=lambda c: c.history[-1]["profit"])
    if worst.history[-1]["profit"] < 0: add("📉", f"«{worst.name}» — крупнейший убыток месяца: {worst.history[-1]['profit']:,.0f} ₽.", "warn")
    notes = []
    for c in hl:
        h = c.history[-1]
        if len(c.history) > 1:
            prev_p = c.history[-2]["price"]
            if h["price"] > prev_p*1.05 and factor < 1.0: notes.append(f"«{c.name}» подняла цену в падающем рынке — ждём потери доли")
            if h["inventory"] > 0.3*max(1,h["prod"]): notes.append(f"у «{c.name}» склад съедает {h['storage']:,.0f} ₽/мес")
        if h.get("vac_pct",0) > 20: notes.append(f"«{c.name}» не закрывает {h['vac_pct']:.0f}% вакансий")
    rep_leader = max(hl, key=lambda c: c.reputation)
    if rep_leader.reputation > 0.5: notes.append(f"«{rep_leader.name}» лидирует по репутации ({rep_leader.reputation:.1f})")
    for nt in notes[:3]: add("📊", nt, "info", "Аналитика")
...
def build_advice(game, c):
    ...
    if c.history:
        ...
        if last["inventory"] > 0.2*max(1,last["prod"]): rows.append(("Склад", f"Избыток {last['inventory']} ед. — снижайте цену или выпуск."))
        if last["price"] > game["avg_price"]*1.1: rows.append(("Цена", "Цена выше среднерыночной >10% — урежет долю, если не обоснована качеством."))
        if last["prod"] > 0 and last["price"] < unit: rows.append(("Цена", "Цена ниже себестоимости — каждая продажа убыточна."))
        if last["util"] < 0.6: rows.append(("Мощность", "Загрузка низкая — не расширяйте мощность, наращивайте спрос."))
        if last.get("vac_pct",0) > 20: rows.append(("Кадры", f"Вакансии {last['vac_pct']:.0f}% — производство упирается в людей."))
        if c.tension > 60: rows.append(("Кадры", f"Напряжённость {c.tension:.0f}/100 — риск забастовки: премия, лояльность, отдых."))
        if c.reputation < 2: rows.append(("Репутация", "Низкая репутация удорожает маркетинг и найм."))
        if c.cash < 3*FIXED_COST: rows.append(("Ликвидность", "Касса мала — держите подушку ≥ 3 месячных постоянных расходов."))
        q_edge = (c.quality-avg_q)/max(1.0,avg_q)
        rec_price = round(min(max(unit*1.35, game["avg_price"]*(1+0.5*q_edge)), game["avg_price"]*1.3),1)
        next_m = (game["turn"]%12)+1
        exp_sales = last["sales"]*MONTH_SEASON[next_m-1]
        rec_prod = int(max(0, min(c.capacity()*0.9, exp_sales*1.05-last["inventory"]*0.6)))
        exp_rev = rec_price*rec_prod
        rec = dict(price=rec_price, prod=rec_prod, mkt=round(min(max(exp_rev*0.10,300),8000),-2),
                   inv=round(min(c.cash*0.20, c.assets*DEPRECIATION+4000),-2) if (last["util"]>=0.85 and c.cash>15000) else round(c.assets*DEPRECIATION,-2),
                   rnd=round(min(c.cash*0.10,4000),-2) if (c.cum_rnd<PATENT_THRESHOLD and c.cash>15000) else round(min(c.cash*0.03,1500),-2),
                   char=500 if c.reputation<3 else 300,
                   hire="; ".join(f"{PROF_RU[p]} +{c.vacancies[p]}" for p in PROFS if c.vacancies.get(p,0)>0) or "не требуется",
                   fin=(f"взять кредит ≈ {round(3*FIXED_COST-c.cash,-2):,.0f} ₽" if c.cash<3*FIXED_COST else (f"положить депозит ≈ {round(c.cash*0.3,-2):,.0f} ₽" if c.cash>25000 and last["util"]<0.8 else "кредит/депозит не требуются")))
    if rec:
        rows.append(("── ЦЕЛЕВЫЕ ЦИФРЫ НА ЭТОТ ХОД ──",""))
        rows.append(("Цена", f"{rec['price']:.1f} ₽/ед.")); rows.append(("Производство", f"{rec['prod']} ед."))
        rows.append(("Маркетинг", f"{rec['mkt']:,.0f} ₽")); rows.append(("Инвестиции", f"{rec['inv']:,.0f} ₽"))
        rows.append(("НИОКР", f"{rec['rnd']:,.0f} ₽")); rows.append(("Благотворительность", f"{rec['char']:,.0f} ₽"))
        rows.append(("Найм", rec["hire"])); rows.append(("Финансы", rec["fin"]))
...
if "token" not in st.session_state: st.session_state.token = secrets.token_hex(4)
...
if over and not game.get("hof_written"):
    ...
    hof_add({"date":datetime.now().strftime("%d.%m.%Y"),"company":winner.name,"mpi":round(winner.mpi_total,1),
             "turns":game["turn"],"humans":len(humans),"sandbox":bool(game.get("sandbox")),"titles":compute_titles(game)})
...
if st.session_state.get("print_doc"):
    ...
    st.markdown(f"<div class='print-head'><div class='ph-t'>Вознесенский муниципальный округ · симулятор «Мастер Капитала»</div>"
    ...
    if kind == "rank":
        ...
        if c.history:
            last = c.history[-1]
            inv_val = last["inventory"]*last["unit_cost"]; loans_sum = sum(l[0] for l in c.loans)
        ...
        for n in game.get("news", []): st.markdown(clip_html(n, month_label), unsafe_allow_html=True)
    elif kind == "diploma":
        rank_pos = sorted([x for x in game["companies"] if x.history], key=lambda x: x.mpi_total, reverse=True)
        for i, cc in enumerate(rank_pos[:3]): st.markdown(diploma_html(game, cc, i+1), unsafe_allow_html=True)
        ...
        for i, (t, body) in enumerate(enc_sections()):
            st.markdown(f"<div class='paper' style='--pc:{PAL[i%5]}'><div class='paper-h'>{t}</div>"
                        f"<div class='paper-s'>Энциклопедия игры «Мастер Капитала»</div></div>", unsafe_allow_html=True)
            st.markdown(body)
...
with st.sidebar:
    ...
    if st.button("Сохранить игру"):
        save_game(game, slot_name); st.success(f"Сохранено: saves/{slot_name}_<token>.json")
...
with tabs[0]:
    if viewer.history:
        last = viewer.history[-1]; prev = viewer.history[-2] if len(viewer.history) > 1 else None
        ...
        with g3, st.container(border=True):
            ...
            fig = go.Figure()
            fig.add_trace(go.Indicator(mode="gauge+number", value=round(last["util"]*100), number=dict(suffix="%"), title=dict(text="Загрузка"), domain=dict(x=[0.02,0.45],y=[0.05,0.95])))
            fig.add_trace(go.Indicator(mode="gauge+number", value=round(min(100,last["inventory"]/max(1,last["capacity"])*100)), number=dict(suffix="%"), title=dict(text="Склад"), domain=dict(x=[0.55,0.98],y=[0.05,0.95])))
            fig_base(fig,300); st.plotly_chart(fig, use_container_width=True)
        ...
        with g4, st.container(border=True):
            ...
            rows = [dict(name=c.name, turn=h["turn"], sales=h["sales"]) for c in game["companies"] for h in c.history]
    ...
    if game["phase"] == "decisions" and not over:
        ...
        if pending:
            ...
            if len(humans_alive) == 1 or st.session_state.gate == nm:
                ...
                if fd:
                    ft = {"positive":"рост спроса / снижение затрат","negative":"падение спроса / рост затрат","stable":"стабильная динамика"}[fd]
                    acc = "100% (перепись)" if game.get("census") else "90%"
                    st.info(f"◆ Прогноз «Рыночного вестника»: **{ft}**. Точность {acc}.")
                ...
                with st.expander(f"🧠 Консультация «Стратегия+» ({consult_cost:,.0f} ₽)"):
                    if can_consult:
                        if st.button("Заказать отчёт-подсказку на этот ход", key=nm+"_consult"):
                            ...
                            game["consult_doc"] = build_advice(game, company); game["consult_doc_turn"] = game["turn"]
                            st.rerun()
                    else:
                    ...
                    if game.get("consult_doc") and game.get("consult_doc_turn") == game["turn"]:
                        st.markdown(paper_html("ОТЧЁТ КОНСАЛТИНГОВОЙ ФИРМЫ «СТРАТЕГИЯ+»", f"для {company.name} · ход {game['turn']}",
                                               game["consult_doc"], PAL[0], stamp=STAMP_CONSULT), unsafe_allow_html=True)
                with st.form("dec_"+nm):
                    ...
                    with fc, st.container():
                        ...
                        with st.container():
                            ...
                            if loan > 0: st.caption(f"Вернуть: **{loan*(1+game.get('loan_rate',LOAN_RATE)*loan_term):,.0f} ₽**")
                            ...
                            if deposit > 0: st.caption(f"Получите: **{deposit*(1+game.get('deposit_rate',DEPOSIT_RATE)*deposit_term):,.0f} ₽**")
                    ...
                    if tnd and tnd.get("open"):
                        with st.container():
                            ...
                            with st.container():
                                ...
                                st.markdown(f"<div class='chips'><span class='chip'>🏛 {tnd['customer']}</span><span class='chip'>Объём {tnd['monthly_vol']} ед./мес ×3 мес</span><span class='chip'>Потолок {tnd['price_cap']:.1f} ₽</span><span class='chip'>Качество ≥{tnd['quality_min']}</span><span class='chip'>Аванс {TENDER_ADVANCE*100:.0f}%</span></div>", unsafe_allow_html=True)
...
with tabs[4]:
    if viewer.history:
        ...
        st.markdown(f"<div class='ribbon'></div><div class='chips'><span class='chip'>📖 стр. {pg+1} из 5</span><span class='chip'>{PAGES[pg]}</span><span class='chip'>◆ {viewer.name}</span></div>", unsafe_allow_html=True)
        inv_val = last["inventory"]*last["unit_cost"]; loans_sum = sum(l[0] for l in viewer.loans)
        total_assets = last["cash"]+last["assets"]+inv_val+last["dep_total"]; equity = total_assets-loans_sum
        if pg == 0:
...
with tabs[6]:
    ...
    doc = st.selectbox("Что печатаем", ["Рейтинг (общий зачёт)","Отчёт команды (полный)","Комплект команды: отчёт + рейтинг",
                                        "Газета (текущий выпуск)","🎓 Диплом победителя (топ-3 + звания)","Правила игры (брошюра)"], key="print_kind")
    comp_name = None
